#if defined(ARDUINO) && ARDUINO >= 100
#include <Arduino.h>
#else
#include <WProgram.h>
#endif

#include <avr/pgmspace.h>
#include "SimplePrint.h"
#include "SimpleFormat.h"
#include <SoftwareSerial.h>

//Debug
#define _DEBUG
#define _DEBUG_REQUEST
#define _DEBUG_RESPONSE
#define _DEBUG_READ_REGISTER

//Start Address
#define START_ADDRESS 9
//Header Length
#define HEADER_LENGTH 3
//Register Length
#define REGISTER_LENGTH 16

//Timestamp.
unsigned long timestamps[2];

//Switch Pin
int switchPins[4] = {2,3,4,5};
//Voltage Pin
int voltagePins[4] = {A0,A1,A2,A3};
//Software Serial
//RX -> Pin8, TX -> Pin9
SoftwareSerial hc05Module(8,9);
//RX -> Pin10, TX -> Pin11
SoftwareSerial smartMeter(10,11);

//CRC Hi
static const unsigned char auchCRCHi[] = {

} ;
  
static const unsigned char auchCRCLo[] = {

} ;

unsigned int CRC16(unsigned char *buffer,int length)
{
    unsigned int value;
    unsigned int uIndex;
    unsigned char uchCRCHi = 0xff;
    unsigned char uchCRCLo = 0xff;
    //Do while.
    for(int i = 0;i < length;i ++)
    {
        uIndex = ((uchCRCLo & 0xFF) ^(buffer[i] & 0xFF)) & 0xFF;
        uchCRCLo = (uchCRCHi & 0xFF) ^ (auchCRCHi[uIndex] & 0xFF);
        uchCRCHi = auchCRCLo[uIndex] & 0xFF;
    }
    value = uchCRCHi & 0xFF;
    value <<= 8;
    value |= uchCRCLo & 0xFF;
    //Return result.
    return value;
    //return (uchCRCHi << 8 | uchCRCLo);
}

float parseFloat(unsigned char *buffer,int position)
{
    unsigned long value;
    //Get value.
    value = 0;
    value <<= 8; value |= buffer[position + 0] & 0xFF;
    value <<= 8; value |= buffer[position + 1] & 0xFF;
    value <<= 8; value |= buffer[position + 2] & 0xFF;
    value <<= 8; value |= buffer[position + 3] & 0xFF;
    //Return result.
    return *((float *)&value);
}

void sendRequest(unsigned int startAddress,unsigned int registerLength)
{
    //CRC16 value.
    unsigned int crc16Value;
    //Buffer
    unsigned char buffer[128];
#ifdef _DEBUG_REQUEST
    //Print result.
    _PRINT0(">ACSample2::sendRequest : try to send request !");
#endif
    //Set buffer.
    buffer[0] = 0x01;//Code of slave.
    buffer[1] = 0x03;//Code of command.
    buffer[2] = startAddress >> 8;
    buffer[3] = startAddress >> 0;
    buffer[4] = registerLength >> 8;
    buffer[5] = registerLength >> 0;

    //Calculate CRC.
    crc16Value = CRC16(buffer,6) & 0xFFFF;
#ifdef _DEBUG_REQUEST
    //Print.
    _PRINT1(">\tCRC16 = 0x%04x",crc16Value);
#endif
    //Set CRC.
    buffer[6] = crc16Value >> 0;
    buffer[7] = crc16Value >> 8;
#ifdef _DEBUG_REQUEST
    //Print.
    Serial.print(">\tbytes = 0x");
    //Print.
    for(int i = 0;i < 8;i ++)
    {
        if(buffer[i] >= 0x00 && buffer[i] <= 0x0F) Serial.print('0'); Serial.print(buffer[i],HEX);
    }
    //Print.
    Serial.println();
#endif
    //Write buffer.
    smartMeter.write(buffer,8);
#ifdef _DEBUG_REQUEST
    //Print.
    _PRINT0(">ACSample2::sendRequest : request was sent !");
#endif
}

int receiveResponse(unsigned char* buffer)
{
    //Position
    int position = 0;
#ifdef _DEBUG_RESPONSE
    //Print.
    _PRINT0(">ACSample2::readRegisters : response received !");
#endif

    //Wait for a while.
    while(smartMeter.available() <= 0);
    //Read.
    buffer[position] = smartMeter.read();
    //Check result.
    if(buffer[position] == 0x01)
    {
#ifdef _DEBUG_RESPONSE      
        _PRINT0(">\tslaveCode = 0x01");
#endif
    }
    else
    {
        //Print result.
        _PRINT1(">ACSample2::readRegisters : fail to read salve code(%d) !",buffer[position]); return -1;
    }
    //Add position.
    position ++;

    //Wait for a while.
    while(smartMeter.available() <= 0);
    //Read.
    buffer[position] = smartMeter.read();
    //Check result.
    if(buffer[position] == 0x03)
    {
#ifdef _DEBUG_RESPONSE      
        _PRINT0(">\tcommandCode = 0x03");
#endif        
    }
    else
    {
        //Print result.
        _PRINT1(">ACSample2::readRegisters : fail to read command code(%d) !",buffer[position]); return -1;
    }
    //Add position.
    position ++;

    //Wait for a while.
    while(smartMeter.available() <= 0);
    //Read.
    buffer[position] = smartMeter.read();
    //Get data length.
    int dataLength = buffer[position] & 0xFF;
#ifdef _DEBUG_RESPONSE    
    _PRINT1(">\tdataLength = %d",dataLength);
#endif
    //Add position.
    position ++;
    //Read.
    for(int i = 0;i < dataLength + 2;i ++)
    {
        //Wait for a while.
        while(smartMeter.available() <= 0);
        //Read.
        buffer[position ++] = smartMeter.read();
    }
#ifdef _DEBUG_RESPONSE
    //Print.
    Serial.print(">\tbytes = 0x");
    //Print.
    for(int i = 0;i < position;i ++)
    {
        if(buffer[i] >= 0x00
          && buffer[i] <= 0x0F) Serial.print('0'); Serial.print(buffer[i],HEX);
    }
    //Print.
    Serial.println();
#endif
    //Return length.
    return position;
}

bool readRegisters()
{
    //Buffer.
    unsigned char buffer[128];

    //Clear port.
    while(smartMeter.available() > 0)
    {
        //Read.
        smartMeter.read();
#ifdef _DEBUG_READ_REGISTER
      //Print result.
      _PRINT0(">ACSample2::readRegisters : there are bytes remaining !");
#endif        
    }

    //Send request.
    sendRequest(START_ADDRESS,REGISTER_LENGTH);

    //Get start time.
    unsigned long startTime = millis();
    //Do while.
    do
    {
        //Listen.
        smartMeter.listen();
        //Check start time.
        if(millis() - startTime > 1000)
        {
#ifdef _DEBUG_READ_REGISTER
            //Print result.
            _PRINT0(">ACSample2::readRegisters : listen timeout !");
#endif
            return false;
        }
      
    }while(smartMeter.available() <= 0);

    //Clear buffer.
    memset(buffer,0,128);
    //Receive response.
    int length = receiveResponse(buffer);
    //Check result.
    if(length < 5)
    {
#ifdef _DEBUG_READ_REGISTER
        _PRINT0(">ACSample2::readRegisters : fail to receive response !");
#endif
        return false;
    }
#ifdef _DEBUG_READ_REGISTER    
    //Check CRC16 result.
    if(CRC16(buffer,length) != 0)
    {
        //Print result.
        _PRINT0(">ACSample2::readRegisters : CRC16 is not correct !"); return false;
    }
#endif

#ifdef _DEBUG_READ_REGISTER
    //Print result.
    _PRINT0(">ACSample2::readRegisters : dump data !");
    for(int i = 0;i < REGISTER_LENGTH / 2;i ++)
    {
        Serial.print(">\tfloat[");
        Serial.print(i);Serial.print("] = ");
        Serial.println(parseFloat(buffer,HEADER_LENGTH + 2 + 4 * i),3);
    }
#endif
    hc05Module.print("M01|");
    for(int i = 0;i < REGISTER_LENGTH / 2;i ++)
    {
        hc05Module.print(
          parseFloat(buffer,HEADER_LENGTH + 2 + 4 * i),3);
        if(i < REGISTER_LENGTH / 2 - 1) hc05Module.print(',');
    }
    hc05Module.println("");
    //Return true.
    return true;
}

bool readPhaseVoltages()
{
    //Buffer.
    unsigned char buffer[128];

    //Clear port.
    while(smartMeter.available() > 0)
    {
        //Read.
        smartMeter.read();
#ifdef _DEBUG_READ_REGISTER
      //Print result.
      _PRINT0(">ACSample2::readPhaseVoltages : there are bytes remaining !");
#endif        
    }

    //Send request.
    sendRequest(0x09,6);

    //Get start time.
    unsigned long startTime = millis();
    //Do while.
    do
    {
        //Listen.
        smartMeter.listen();
        //Check start time.
        if(millis() - startTime > 1000)
        {
#ifdef _DEBUG_READ_REGISTER
            //Print result.
            _PRINT0(">ACSample2::readPhaseVoltages : listen timeout !");
#endif
            return false;
        }
      
    }while(smartMeter.available() <= 0);

    //Clear buffer.
    memset(buffer,0,128);
    //Receive response.
    int length = receiveResponse(buffer);
    //Check result.
    if(length < 5)
    {
#ifdef _DEBUG_READ_REGISTER
        _PRINT0(">ACSample2::readPhaseVoltages : fail to receive response !");
#endif
        return false;
    }
#ifdef _DEBUG_READ_REGISTER    
    //Check CRC16 result.
    if(CRC16(buffer,length) != 0)
    {
        //Print result.
        _PRINT0(">ACSample2::readPhaseVoltages : CRC16 is not correct !"); return false;
    }
#endif

#ifdef _DEBUG_READ_REGISTER
    //Print result.
    _PRINT0(">ACSample2::readPhaseVoltages : dump data !");
    for(int i = 0;i < 3;i ++)
    {
        Serial.print(">\tfloat[");
        Serial.print(i);Serial.print("] = ");
        Serial.println(parseFloat(buffer,HEADER_LENGTH + 2 + 4 * i),3);
    }
#endif
    hc05Module.print("M01|Up|");
    for(int i = 0;i < 3;i ++)
    {
        hc05Module.print(
          parseFloat(buffer,HEADER_LENGTH + 2 + 4 * i),3);
        if(i < 2) hc05Module.print(','); else hc05Module.println();
    }
    //Return true.
    return true;
}

bool readFrequence()
{
    //Buffer.
    unsigned char buffer[128];

    //Clear port.
    while(smartMeter.available() > 0)
    {
        //Read.
        smartMeter.read();
#ifdef _DEBUG_READ_REGISTER
      //Print result.
      _PRINT0(">ACSample2::readFrequence : there are bytes remaining !");
#endif        
    }

    //Send request.
    sendRequest(0x2F,2);

    //Get start time.
    unsigned long startTime = millis();
    //Do while.
    do
    {
        //Listen.
        smartMeter.listen();
        //Check start time.
        if(millis() - startTime > 1000)
        {
#ifdef _DEBUG_READ_REGISTER
            //Print result.
            _PRINT0(">ACSample2::readFrequence : listen timeout !");
#endif
            return false;
        }
      
    }while(smartMeter.available() <= 0);

    //Clear buffer.
    memset(buffer,0,128);
    //Receive response.
    int length = receiveResponse(buffer);
    //Check result.
    if(length < 5)
    {
#ifdef _DEBUG_READ_REGISTER
        _PRINT0(">ACSample2::readFrequence : fail to receive response !");
#endif
        return false;
    }
#ifdef _DEBUG_READ_REGISTER    
    //Check CRC16 result.
    if(CRC16(buffer,length) != 0)
    {
        //Print result.
        _PRINT0(">ACSample2::readFrequence : CRC16 is not correct !"); return false;
    }
#endif

#ifdef _DEBUG_READ_REGISTER
    //Print result.
    _PRINT0(">ACSample2::readFrequence : dump data !");
    Serial.print(">\tfloat[0] = ");
    Serial.println(parseFloat(buffer,HEADER_LENGTH + 2),3);
#endif
    hc05Module.print("M01|Uf|");
    hc05Module.println(parseFloat(buffer,HEADER_LENGTH + 2),3);
    //Return true.
    return true;
}

void doVoltageSamples()
{
    //Voltage values.
    int voltageValues[100];
    
    //Read voltage.
    for(int i = 0;i < 100;i ++)
    {
        delayMicroseconds(300);
        voltageValues[i] = analogRead(voltagePins[0]);
    }
    hc05Module.print("V|");
    //Do output.
    for(int i = 0;i < 100;i ++)
    {
        hc05Module.
          print(voltageValues[i]);
        if(i < 99) hc05Module.print(',');
    }
    hc05Module.println();

    delay(5);

    //Read voltage.
    for(int i = 0;i < 100;i ++)
    {
        delayMicroseconds(300);
        voltageValues[i] = analogRead(voltagePins[1]);
    }
    hc05Module.print("Va|");
    //Do output.
    for(int i = 0;i < 100;i ++)
    {
        hc05Module.
          print(voltageValues[i]);
        if(i < 99) hc05Module.print(',');
    }
    hc05Module.println();

    delay(5);

    //Read voltage.
    for(int i = 0;i < 100;i ++)
    {
        delayMicroseconds(300);
        voltageValues[i] = analogRead(voltagePins[2]);
    }
    hc05Module.print("Vb|");
    //Do output.
    for(int i = 0;i < 100;i ++)
    {
        hc05Module.
          print(voltageValues[i]);
        if(i < 99) hc05Module.print(',');
    }
    hc05Module.println();

    delay(5);

    //Read voltage.
    for(int i = 0;i < 100;i ++)
    {
        delayMicroseconds(300);
        voltageValues[i] = analogRead(voltagePins[3]);
    }
    hc05Module.print("Vc|");
    //Do output.
    for(int i = 0;i < 100;i ++)
    {
        hc05Module.
          print(voltageValues[i]);
        if(i < 99) hc05Module.print(',');
    }
    hc05Module.println();

    delay(5);
}

void setup()
{
    //Open serial communication and wait for port to open.
    Serial.begin(115200);
    //Do while.
    while(!Serial) 
    {
        //Wait for serial port to connect.
        //Need for leonardo.
    }
#ifdef _DEBUG
    // Print.
    _PRINT0(">ACSample2::setup : begin to work !");
#endif

    //Declare the switch pin as an INPUT.
    for(int i = 0;i < 4;i ++)
        pinMode(switchPins[i],INPUT);
#ifdef _DEBUG
    //Print.
    _PRINT0(">ACSample2::setup : switch pin connected !");
#endif
    
    //Declare the voltage pin as an INPUT.
    for(int i = 0;i < 4;i ++)
        pinMode(voltagePins[i],INPUT);
#ifdef _DEBUG
    //Print.
    _PRINT0(">ACSample2::setup : voltage pin connected !");
#endif
    
    //Setup baudrate for serial port.
    smartMeter.begin(9600);
#ifdef _DEBUG
    //Print.
    _PRINT0(">ACSample2::setup : smart meter connected !");
#endif

    //Setup baudrate for serial port.
    hc05Module.begin(115200);
#ifdef _DEBUG
    //Print.
    _PRINT0(">ACSample2::setup : bluetooth module connected !");
#endif

    //Set timestamp.
    timestamps[0] = timestamps[1] = millis();
#ifdef _DEBUG　
    //Print.
    _PRINT0(">ACSample2::setup : device was setup !");
#endif
}

void loop()
{
    //Get current time.
    unsigned long currentTime = millis();
    //Check result.
    if(currentTime - timestamps[0] > 1000)
    {
        //Set timestamp.
        timestamps[0] = millis();
        //Do voltage values.
        doVoltageSamples(); delay(20);
    }
    
    //Check current time.
    if(currentTime - timestamps[1] > 1000)
    {
        //Set timestamp.
        timestamps[1] = millis();
        //Read phase voltages.
        readPhaseVoltages(); delay(20);
        //Read frequence.
        readFrequence(); delay(20);

        int switchValues[4];
        //Try to read value from switch pin.
        for(int i = 0;i < 4;i ++)
        {
            switchValues[i] = digitalRead(switchPins[i]);
        }
#ifdef _DEBUG
        //Print result.
        _PRINT0(">ACSample2::loop : show status !");
        _PRINT1(">\ttimestamp = %u",currentTime);
        _PRINT4(">\tswitch values = %d,%d,%d,%d",
            switchValues[0],switchValues[1],switchValues[2],switchValues[3]);
#endif
        hc05Module.print("S01|");
        for(int i = 0;i < 4;i ++)
        {
            hc05Module.
                print(switchValues[i]);
            if(i < 3) hc05Module.print(',');
        }
        hc05Module.println();
    }
}
